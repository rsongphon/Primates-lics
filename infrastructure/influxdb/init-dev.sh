...
# This script fully sets up InfluxDB since we disabled Docker's auto-setup
...
# Use INFLUX_HOST if set (for setup container), otherwise use localhost
INFLUX_URL=${INFLUX_HOST:-http://localhost:8086}
until wget -q --spider ${INFLUX_URL}/health 2>/dev/null; do
...
# Check if InfluxDB is already set up by checking the setup endpoint
# If setup is allowed, it returns "allowed: true", otherwise "allowed: false"
if wget -q -O - "${INFLUX_URL}/api/v2/setup" 2>/dev/null | grep -q '"allowed":false'; then
  ...
  influx setup \
    ...
    --retention "${INFLUXDB_INIT_RETENTION}" \
    --token "${INFLUXDB_INIT_ADMIN_TOKEN}" \
    --force || echo "Setup may have already been completed"
fi

# Configure environment for subsequent commands
export INFLUX_TOKEN=${INFLUXDB_INIT_ADMIN_TOKEN}
export INFLUX_ORG=${INFLUXDB_INIT_ORG}
# INFLUX_HOST is already set from environment or defaults to http://localhost:8086
...
influx bucket create --name telemetry --retention 7d --description "Telemetry data (legacy/test compatibility)" || echo "Bucket 'telemetry' already exists"
...
influx auth create \
  ...
  --read-bucket "${INFLUXDB_INIT_BUCKET}" \
  --write-bucket "${INFLUXDB_INIT_BUCKET}" 2>/dev/null || echo "Backend token may already exist"

influx auth create \
  --org "${INFLUXDB_INIT_ORG}" \
  --description "Edge Agent Token (Dev)" \
  --write-bucket "${INFLUXDB_INIT_BUCKET}" \
  --write-bucket experiment-data-dev \
  --write-bucket system-metrics-dev 2>/dev/null || echo "Edge agent token may already exist"
...
influx write \
  --bucket "${INFLUXDB_INIT_BUCKET}" \
  --precision s \
  "device_telemetry,device_id=dev-rpi-001,metric=temperature value=23.5 ${TIMESTAMP}"

influx write \
  --bucket "${INFLUXDB_INIT_BUCKET}" \
  --precision s \
  "device_telemetry,device_id=dev-rpi-001,metric=humidity value=45.2 ${TIMESTAMP}"
...
echo "   - Main bucket: ${INFLUXDB_INIT_BUCKET} (retention: ${INFLUXDB_INIT_RETENTION})"
